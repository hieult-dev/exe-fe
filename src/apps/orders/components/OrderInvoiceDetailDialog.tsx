...
import type { OrderListItemDTO } from "@/apps/orders/model"
import { formatCurrencyVND, formatDateTimeViVN } from "@/common/utils/format"

type OrderInvoiceDetailDialogProps = {
  visible: boolean
  loading: boolean
...
export function OrderInvoiceDetailDialog({ visible, loading, invoice, order, onHide }: OrderInvoiceDetailDialogProps) {
  const customerName = order?.customer?.fullName || "Khách lẻ"
  ...
  const lineTypeBody = (line: InvoiceLineDTO) => <Tag value={LINE_TYPE_LABEL[line.lineType] || line.lineType} severity="info" />
  ...
  return (
    <Dialog
      visible={visible}
      onHide={onHide}
      header={`Hóa đơn ${invoice ? `#${invoice.id}` : order?.orderCode || ""}`}
    ...
    >
      {loading ? (
        ...
        <div className="flex min-h-[180px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 text-center">
          ...
          <p className="m-0 text-sm font-semibold text-slate-600">Không tìm thấy hóa đơn cho đơn hàng này.</p>
        </div>
      )}
    </Dialog>
  )
